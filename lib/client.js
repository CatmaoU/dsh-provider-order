...
(function () {
  ...
  const CSS = [
    ...
    // 实际 DOM 类名是 CSS module 哈希 + 稳定后缀（如 `.VOzbGW_panel`、`.VOzbGW_nav`、
    // `.VOzbGW_navList`），因此不能用普通 `.nav`，必须用属性选择器匹配 `_nav` 后缀。
  ...
  function setupSettingsScroll() {
    try {
      const nav = document.querySelector('[class*="_panel"] > [class*="_nav"]');
      if (!nav) return;

      const containerHeight = nav.clientHeight || 600;
      const contentHeight = nav.scrollHeight;

      // 如果内容高度超过容器高度，启用滚动；未溢出时保持 auto（不显示滚动条）
      nav.style.overflowY = contentHeight > containerHeight ? 'auto' : '';
      nav.style.scrollbarGutter = contentHeight > containerHeight ? 'stable' : '';
      console.log(
        '[dsh-provider-order] 设置面板导航栏' + (contentHeight > containerHeight ? '已启用滚动' : '无需滚动') +
        '（内容高度:', contentHeight, 'px，容器高度:', containerHeight, 'px）',
      );
    } catch (e) {
