...
(function () {
  ...
  const CSS = [
    ...
    // ── 设置面板导航栏滚动（插件过多时左侧菜单可下滑）────────────────────────
    // .nav 容器：固定宽度 188px，内容超出时纵向滚动，滚动条使用 l2 层级样式。
    // .navList：flex:1 + min-height:0 使其成为 flex 子项中的可滚动区域，
    //           标题行 (.navTitle) 始终固定在顶部不随列表滚动。
  ...
  // ── 自动检测设置面板导航栏滚动（插件过多时左侧菜单可下滑）──────────────────
  // 动态计算内容高度，确保超过容器高度时显示滚动条
  function setupSettingsScroll() {
    try {
      const nav = document.querySelector('[data-plugin="settings"] .nav');
      if (!nav) return;

      const computedStyle = window.getComputedStyle(nav);
      const containerHeight = parseInt(computedStyle.height, 10) || 600;
      const contentHeight = nav.scrollHeight;

      // 如果内容高度超过容器高度，启用滚动
      if (contentHeight > containerHeight) {
        // 确保容器有明确的高度限制
        if (!nav.style.maxHeight || nav.style.maxHeight === 'none') {
          nav.style.maxHeight = containerHeight + 'px';
        }
        // 确保 overflow 已启用
        if (computedStyle.overflowY !== 'auto' && computedStyle.overflowY !== 'scroll') {
          nav.style.overflowY = 'auto';
        }
        // 确保 scrollbar-gutter 已设置
        if (computedStyle.scrollbarGutter !== 'stable') {
          nav.style.scrollbarGutter = 'stable';
        }
        console.log('[dsh-provider-order] 设置面板导航栏已启用滚动（内容高度:', contentHeight, 'px，容器高度:', containerHeight, 'px）');
      } else {
        // 内容未溢出，移除可能存在的 max-height 限制
        nav.style.maxHeight = '';
        console.log('[dsh-provider-order] 设置面板导航栏无需滚动（内容高度:', contentHeight, 'px，容器高度:', containerHeight, 'px）');
      }
    } catch (e) {
      console.warn('[dsh-provider-order] 设置面板导航栏滚动设置失败:', e);
    }
  }
