...
function apply(ctx, config) {
  ...
  if (webServer && typeof webServer.register === "function") {
    ...
    // 供客户端做「DOM 行 ↔ provider id」映射的全量列表（与设置页渲染顺序一致：
    // directory 目录序在前，live-only 注册追加尾部）。
    ctx.effect(
      () =>
        webServer.register({
          kind: "exact",
          path: "/dsh-provider-order/providers",
          handler: async (req, res) => {
            try {
              const llm = ctx.llm;
              const registered =
                typeof llm.listProviders === "function" ? llm.listProviders() : [];
              const active = new Set(registered.map((p) => p && p.id));
              const directory =
                typeof llm.listConfigurableProviders === "function"
                  ? llm.listConfigurableProviders()
                  : [];
              const declared = new Set(directory.map((e) => e && e.provider));
              const providers = directory.map((e) => ({
                provider: e.provider,
                displayName: e.displayName ?? e.provider,
              ...
              for (const p of registered) {
                if (p && declared.has(p.id)) continue;
                providers.push({
                  provider: p.id,
